...
var initialiseCustomIconForm = function (settings) {
    ...
    const useRow = $('<div class="flex items-start justify-between gap-4"></div>')
        ...
        .append($('<div class="w-28 flex-shrink-0"></div>')
            .append($('<input type="checkbox" id="toggle-use-custom-icon" class="hidden disabled:cursor-not-allowed">').prop('checked', settings.config.customIconPosition))
        );
    body.append(useRow);
    ...
    const typeRow = $('<div class="flex items-start justify-between gap-4"></div>')
        ...
        .append($('<div class="w-32 flex-shrink-0"></div>')
            .append($('<input type="checkbox" id="toggle-icon-type" class="hidden disabled:cursor-not-allowed">').prop('checked', settings.injectedIconConfig.type === 'anchored')));
    body.append(typeRow);
    ...
    const sideCol = $('<div class="space-y-2"></div>')
        ...
        .append($('<input type="checkbox" id="toggle-side" class="hidden disabled:cursor-not-allowed">').prop('checked', settings.injectedIconConfig.side === 'left'));
    const sideOffsetCol = $('<div class="space-y-2"></div>')
        ...
        .append($('<div class="flex items-center gap-2"></div>')
            .append($(`<input type="text" id="side-offset" class="w-20 rounded-md border border-slate-600 bg-slate-800 px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:cursor-not-allowed" maxlength="4">`).val(getAmountFromOffset(settings.injectedIconConfig.sideOffset)))
            .append($('<input type="checkbox" id="toggle-side-offset" class="hidden disabled:cursor-not-allowed">').prop('checked', getUnitFromOffset(settings.injectedIconConfig.sideOffset) === 'px'))
    ...
    const posCol = $('<div class="space-y-2"></div>')
        ...
        .append($('<input type="checkbox" id="toggle-position" class="hidden disabled:cursor-not-allowed">').prop('checked', settings.injectedIconConfig.position === 'top'));
    const posOffsetCol = $('<div class="space-y-2"></div>')
        .append($('<label for="position-offset" class="font-medium">Vertical offset</label>'))
        .append($('<div class="flex items-center gap-2"></div>')
            .append($(`<input type="text" id="position-offset" class="w-20 rounded-md border border-slate-600 bg-slate-800 px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:cursor-not-allowed" maxlength="4">`).val(getAmountFromOffset(settings.injectedIconConfig.positionOffset)))
            .append($('<input type="checkbox" id="toggle-position-offset" class="hidden disabled:cursor-not-allowed">').prop('checked', getUnitFromOffset(settings.injectedIconConfig.positionOffset) === 'px'))
            .append($('<span class="text-xs text-slate-400">Units</span>'))
        );
    ...
    const bgRow = $('<div class="space-y-2 flex gap-4"></div>')
        ...
        .append($(`<input type="text" id="icon-background-color" data-coloris class=" text-white w-40 rounded-md border border-slate-600 bg-slate-800 px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:cursor-not-allowed" />`).val(settings.injectedIconConfig.backgroundColor));
    body.append(bgRow);
...
.servarr-ext_icon-image {
    width: 40px;
    height: 40px;
    background: url('${getIconAsDataUri(siteId, null, null, null)}') no-repeat;
}
