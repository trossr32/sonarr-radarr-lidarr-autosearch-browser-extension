...
test('allocine tv has sonarr icon', async ({ page }) => {
  // waitForServarrIcon reloads if the MV3 service worker missed its one-shot
  // injection (can happen under parallel load) — see helper for details.
  test.slow();
...
test('allocine movie has radarr icon', async ({ page }) => {
  ...
  await waitForServarrIcon(page);
  await expect(page.locator(iconDataLocator)).toHaveCount(1, { timeout: 20000 });
  await expect(page.locator(iconDataLocator)).toHaveAttribute('href', getExpectedRadarrUrl('the%20dark%20knight%2C%20le%20chevalier%20noir'), { ignoreCase: true, timeout: 20000 });
});
