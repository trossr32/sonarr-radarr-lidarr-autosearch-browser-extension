...
import { getExpectedReadarrUrl } from '../helpers';
...
test('goodreads book has readarr icon', async ({ page }) => {
  ...
  await expect(page.locator(iconDataLocator)).toHaveAttribute('href', getExpectedReadarrUrl('the%20great%20gatsby'), { ignoreCase: true });
});
