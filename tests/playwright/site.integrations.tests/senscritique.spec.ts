...
test('senscritique tv has sonarr icon', async ({ page }) => {
  await page.goto('https://www.senscritique.com/serie/fringe/164968');
...
test('senscritique movie has radarr icon', async ({ page }) => {
  await page.goto('https://www.senscritique.com/film/the_dark_knight_le_chevalier_noir/419456');
  await expect(page.locator(iconDataLocator)).toHaveCount(1);
  await expect(page.locator(iconDataLocator)).toBeVisible();
